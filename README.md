...
[Extra Features](https://github.com/jManAscending/DnDHouseRules#extra-features)  
...
## Nonlethality
Range/melee does not affect lethality.  Psychic damage can be made nonlethal without issue; targets will drop to 0 and be stable.  Bludgeoning damage can be made nonlethal if it is the result of an attack roll, and targets will drop to 0 and be stable.  Necrotic damage canot be specified to be nonlethal.  Piercing, Slashing, and all other types of damage can be made nonlethal if and only if the damage was dealt as the result of an attack roll.  If such an attack reduces the target to 0, the creature drops unconscious and unstable, making death saving throws according to player rules.  PCs at 0 HP will follow RAW for the most part, with one exception - if an NPC makes a nonlethal Bludgeoning or Psychic attack against a PC and they drop to 0, the PC will be immediately stable.
...
If a creature is incapacitated for any reason in the middle of active combat, any attack has advantage, and any hit from within 5ft. is a critical.  
...
If a creature is incapacitated for a limited amount of time outside of combat via a spell or other magical effect (e.g. Hold Person), any attack from within 5ft. will automatically hit, any other attack has advantage, and any hit is a critical.
...
## Extra Features
All spells, all feats, and most backgrounds from the PHB, EE, XGE, and SCAG are available, though some have changes listed below.  Any backgrounds that are setting-specific and any UA material requires DM approval before using.  The current setting uses an extremely customized version of Artificer; it takes precedence over the UA version.  Items not from official WotC material require heavy DM scutiny to be accepted.  
...
## Social Encounters
Active Persuation can only be rolled against a PC if the PC is Charmed/under a Friends spell (in which case the check is made without advantage), or to contest an Insight check.
